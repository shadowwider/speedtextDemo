...
export default React.createClass({
  ...
  getInitialState() {
    return {
      ...
      audioSource: null,
      speakerLabels: false,
      keywords: this.getKeywords('zh-CN_BroadbandModel'),
  ...
  render() {
    ...
    return (
      <Dropzone onDropAccepted={this.handleUserFile} onDropRejected={this.handleUserFileRejection} maxSize={200 * 1024 * 1024} accept="audio/wav, audio/l16, audio/ogg, audio/flac, .wav, .ogg, .opus, .flac" disableClick={true} className="dropzone _container _container_large dialog" activeClassName="dropzone-active" rejectClassName="dropzone-reject" ref={(node) => {
        ...
        <Recon messages={messages} keywords={this.state.settingsAtStreamStart.keywords} isInProgress={!!this.state.audioSource}>      
          </Recon>
      </Dropzone>
